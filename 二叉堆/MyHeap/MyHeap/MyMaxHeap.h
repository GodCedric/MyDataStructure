#ifndef __MYMAXHEAP__H__
#define __MYMAXHEAP__H__
...
//构造析构
template<class T>
MyMaxHeap<T>::MyMaxHeap(int size):maxNum(size){
	curNum = 0;
	mHeap = new T[size];
}

template<class T>
MyMaxHeap<T>::~MyMaxHeap(){
	maxNum = 0;
	curNum = 0;
	delete[] mHeap;
}
...
//返回元素索引
template<class T>
int MyMaxHeap<T>::getIndex(T data){
...
//插入元素
template<class T>
bool MyMaxHeap<T>::insert(T data){
	//满了返回-1
	if(curNum == maxNum)
		return false;
	//先插入数组末尾
...
///自底向上维护堆的性质
template<class T>
void MyMaxHeap<T>::filterup(int cur){
	int parent = (cur-1)/2;
	T temp = mHeap[cur];   //先保存需要插入的值
	//寻找需要插入的位置
	while(cur){
		//比较结点与父结点的大小
		if(mHeap[parent] >= temp){  //父结点比较大不用动
			break;
		}else{                              //父结点比较小需要交换位置
...
//删除元素
template<class T>
bool MyMaxHeap<T>::remove(T data){
...
//自顶向下维护堆
template<class T>
void MyMaxHeap<T>::filterdown(int cur){
	int lchild = 2*cur+1;  //左孩子
	T temp = mHeap[cur];   //先存值
	while(lchild <= curNum-1){
		if(lchild < curNum-1){   //选择左右孩子中比较大的那个
