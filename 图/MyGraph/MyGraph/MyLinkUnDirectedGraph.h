#ifndef __MYLINKUNDIRECTEDGRAPH__H__
#define __MYLINKUNDIRECTEDGRAPH__H__
...
class MyLinkUnDirectedGraph:public MyGraph<VertexType,EdgeType>{
public:
	//构造析构
	MyLinkUnDirectedGraph(int sz = DefaultVertices);
	~MyLinkUnDirectedGraph();
	//基类纯虚函数实例化
...
//构造函数
template<typename VertexType,typename EdgeType> 
MyLinkUnDirectedGraph<VertexType,EdgeType>::MyLinkUnDirectedGraph(int sz){
	maxVertices = sz;
	numVertices = 0;
	numEdges = 0;
...
//析构函数
template<typename VertexType,typename EdgeType> 
MyLinkUnDirectedGraph<VertexType,EdgeType>::~MyLinkUnDirectedGraph(){
	//释放链表空间
	for(int i=0;i<maxVertices;i++){
...
//返回结点i的值
template<typename VertexType,typename EdgeType>
VertexType MyLinkUnDirectedGraph<VertexType,EdgeType>::getValue(int i){          //获取结点值
...
template<typename VertexType,typename EdgeType> 
EdgeType MyLinkUnDirectedGraph<VertexType,EdgeType>::getWeight(int v1,int v2){
	//前提是结点必须存在
	if(v1!=-1 && v2!=-1){
...
/*//获取结点v的第一个邻接点
template<typename VertexType,typename EdgeType> 
int MyLinkUnDirectedGraph<VertexType,EdgeType>::getFirstNeighbor(int v){
	if(v != -1){
...
//获取结点v的邻接结点w的下一个邻接点
template<typename VertexType,typename EdgeType> 
int MyLinkUnDirectedGraph<VertexType,EdgeType>::getNextNeighbor(int v,int w){   //这里w要是v的一个邻接点
	if(v != -1){
		Edge<typename VertexType,typename EdgeType> *p = NodeTable[v].adj;
...
//插入结点
template<typename VertexType,typename EdgeType> 
bool MyLinkUnDirectedGraph<VertexType,EdgeType>::insertVertex(const VertexType& vertex){    //输入值为结点名
...
//删除结点
template<typename VertexType,typename EdgeType> 
bool MyLinkUnDirectedGraph<VertexType,EdgeType>::removeVertex(int v){
	//先判断下结点v是否合理
...
//删除边
template<typename VertexType,typename EdgeType> 
bool MyLinkUnDirectedGraph<VertexType,EdgeType>::removeEdge(int v1,int v2){
	//先判断下输入是否合理
	if(v1>=0 && v1<numVertices && v2>=0 && v2<numVertices){
		//先把v1的边删掉
...
//图的输入与构造
template<typename VertexType,typename EdgeType> 
void MyLinkUnDirectedGraph<VertexType,EdgeType>::inputGraph(){
...
//图的输出
template<typename VertexType,typename EdgeType> 
void MyLinkUnDirectedGraph<VertexType,EdgeType>::outputGraph(){
...
//图的广度优先搜索
template<typename VertexType,typename EdgeType> 
void MyLinkUnDirectedGraph<VertexType,EdgeType>::BFS(VertexType sv){
...
//图的深度优先搜索
template<typename VertexType,typename EdgeType> 
void MyLinkUnDirectedGraph<VertexType,EdgeType>::DFS_visit(Vertex<typename VertexType,typename EdgeType>& u){
	static int time = 0;
	time = time + 1;
	u.d = time;
	u.color = GRAY;
...
template<typename VertexType,typename EdgeType> 
void MyLinkUnDirectedGraph<VertexType,EdgeType>::DFS(){
