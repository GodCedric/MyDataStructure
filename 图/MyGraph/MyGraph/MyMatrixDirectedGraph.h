#ifndef __MYMATRIXDIRECTEDGRAPH__H__
#define __MYMATRIXDIRECTEDGRAPH__H__

/*********基于矩阵的图类*********/
#include"MyGraph.h"

template<typename VertexType,typename EdgeType> 
class MyMatrixDirectedGraph:public MyGraph<VertexType,EdgeType>{
public:
	//构造析构
	MyMatrixDirectedGraph(int size=DefaultVertices);
	~MyMatrixDirectedGraph();
...
//构造函数
template<typename VertexType,typename EdgeType>
MyMatrixDirectedGraph<VertexType,EdgeType>::MyMatrixDirectedGraph(int size){
	numVertices = 0;
	numEdges = 0;
	maxVertices = size;
	//分配结点矩阵空间
...
//析构函数
template<typename VertexType,typename EdgeType>
MyMatrixDirectedGraph<VertexType,EdgeType>::~MyMatrixDirectedGraph(){
	//释放结点矩阵空间
	delete[] VerticesList;
	//释放边矩阵空间
	for(int i=0;i<maxVertices;i++)
...
//获取结点值
template<typename VertexType,typename EdgeType>
VertexType MyMatrixDirectedGraph<VertexType,EdgeType>::getValue(int i){
	return (i>=0 && i<numVertices)?VerticesList[i]:0;
}

//获取边的权值
template<typename VertexType,typename EdgeType>
EdgeType MyMatrixDirectedGraph<VertexType,EdgeType>::getWeight(int v1,int v2){
	return (v1!=-1 && v2!=-1)?Edge[v1][v2]:0;
}

//插入结点
template<typename VertexType,typename EdgeType>
bool MyMatrixDirectedGraph<VertexType,EdgeType>::insertVertex(const VertexType& vertex){
...
//插入边
template<typename VertexType,typename EdgeType>
bool MyMatrixDirectedGraph<VertexType,EdgeType>::insertEdge(int v1,int v2,EdgeType cost){
...
//删除结点
template<typename VertexType,typename EdgeType>
bool MyMatrixDirectedGraph<VertexType,EdgeType>::removeVertex(int v){
	//判断输入是否合理
...
//删除边
template<typename VertexType,typename EdgeType>
bool MyMatrixDirectedGraph<VertexType,EdgeType>::removeEdge(int v1,int v2){
...
//图输入
template<typename VertexType,typename EdgeType>
void MyMatrixDirectedGraph<VertexType,EdgeType>::inputGraph(){
...
//图输出
template<typename VertexType,typename EdgeType>
void MyMatrixDirectedGraph<VertexType,EdgeType>::outputGraph(){
